...
import pymongo
import pandas as pd
myclient = pymongo.MongoClient("mongodb://administrator:administrator@140.118.70.40:27017")
mydb = myclient["WiFi_AP_Data"]
mycol = mydb["Controller_4"]

data = pd.DataFrame(list(mycol.find().sort('_id',-1).limit(48)))
df = data[['ap_name', 'sta_count']].sort_values(by=['ap_name'])

ap_name = []
no_client = []
for i in range(len(df)):
#     print(df['AP Name'][i])
    ap_name.append(df['ap_name'][i])
    no_client.append(int(df['sta_count'][i]))
...
class App():
  ...
  def __init__(self, master = None):
    ...
    def __generateConfig():
      ...
      for i in range(1, int(App.deviceCount.get()) + 1):
        deviceConfig = DeviceConfig(id = 'Device' + str(i),
          ...
          retentionPolicyName = '')
        for j in range(1, int(App.analogCount.get()) + 1):
          analog = AnalogTagConfig(name = ap_name[j-1],
            description = ap_name[j-1],
            readOnly = False,
            arraySize = 0,
            spanHigh = 1000,
            spanLow = 0,
    ...
    App.analogCount.set(len(ap_name))
